...
class SnakeNN:
    def __init__(self, initial_games = 10000, test_games = 100, goal_steps = 300,gui=False):
    ...
    def initial_population(self):
        ...
        for _ in range(self.initial_games):
            ...
            for _ in range(self.goal_steps):
                ...
                if done:
                    training_data.append([self.add_action_to_observation(prev_observation, action), -5])
                    break
                else:
                    training_data.append([self.add_action_to_observation(prev_observation, action), 5])
                    prev_observation = self.generate_observation(snake)
    ...
    def model(self):
        ...
        network = fully_connected(network, 25, activation='sigmoid')
    ...
    def train_model(self, training_data, model):
        ...
        model.fit(X,y, n_epoch = 1, shuffle = True, run_id = 'snake_nn.tflearn')
        model.save('snake_nn.tflearn')
        return model
    ...
    def play(self):
        nn_model = self.model()
        nn_model.load('snake_nn.tflearn')
...
if __name__ == "__main__":
    ...
    #SnakeNN(gui=False).train()
    SnakeNN(gui=True).play()
